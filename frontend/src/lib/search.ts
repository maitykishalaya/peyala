/**
 * Central Search Utility for Peyala POS (Frontend)
...
/**
 * Normalizes a string: lowercase, accents removed, multiple spaces collapsed, trimmed.
 */
export function normalizeStr(str: SearchTarget): string {
  if (str == null) return '';
  return String(str)
    .toLowerCase()
    .normalize('NFD')
 ...
 * @param targets - One or more target strings or numbers (e.g. [item.name, item.description])
 * @param query - The user search query
 * @returns boolean - True if targets match query
 */
export function matchesSearch(
  targets: SearchTarget | SearchTarget[],
  ...
  const normTargets: string[] = [];
  const normTargetsNoSpace: string[] = [];
...
export function filterBySearch<T>(
  items: T[],
  query: string | null | undefined,
  getTargets?: (item: T) => SearchTarget | SearchTarget[]
): T[] {
  if (!Array.isArray(items)) return [];
  if (!query || !String(query).trim()) return items;
  return items.filter((item) => matchesSearch(getTargets ? getTargets(item) : (item as any), query));
}
...
export function sortBySearchRelevance<T>(
  ...
  return [...items]
    .map((item, index) => ({
      item,
      index,
      score: getSearchScore(getTargets ? getTargets(item) : (item as any), query),
    }))
...
/**
 * Check if a target exactly matches query (ignoring case & whitespace).
 */
export function isExactMatch(target: SearchTarget, query: string | null | undefined): boolean {
